...
def main():
    ...
    try:
        ...
        from adaptagen.core.adaptagen import AdaptaGen
        ...
        if args.specific_version:
            ...
            # For the modular version, we'll use our own AdaptaGen class
            agent_module = None
            version = "0.0.2-r2"  # Current version
        ...
        # Initialize the agent
        if agent_module:
            # Use the loaded module's AdaptaGen class
            AgentClass = getattr(agent_module, 'AdaptaGen')
            ConfigClass = getattr(agent_module, 'Config')
            agent_config = ConfigClass.from_env()
            agent = AgentClass(agent_config)
        else:
            # Use our own AdaptaGen class
            agent = AdaptaGen(config)
